public class ignoreTest {
    ...
    private static void test2(String fileName) {
		...
		// checks amount of neighboring cells
		for (int i = 1; i < cols -1; i++) {
			for (int j = 1; j < rows -1; j++){
				int neighbors = count(board, i, j);
				System.out.println("The value of cell at (" + i + ", " + j + ") has " + neighbors + " neighbors" );
			}
		}

		// checks the cellvalue function
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {
				int value = cellValue(board, i, j);
				System.out.println("Next value for cell at (" + i + ", " + j + ") is " + value);
			}
		}
		// for (int i = 1; i < cols -1; i++) {
		// 	for (int j = 1; j < rows -1; j++){
    ...
    public static int count(int[][] board, int i, int j) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                aliveCount += board[i + x][j + y];
            }
        }
    
        // Subtract the cell itself only if it's alive
        return aliveCount - board[i][j];
    }
    ...
    public static void show(int[][] board) {
        ...
        // StdDraw, the RGB codes of black and white are, respectively, (0,0,0) and (255,255,255).
        for (int i = 1; i < rows - 1; i++) {
            for (int j = 1; j < cols - 1; j++) {
